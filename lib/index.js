...
class Instagram{
    ...
    async getIdContentFromShortCode(codePage){
        try {
            const result = await this.request.get(`${this.baseUrl}graphql/query/?query_hash=6ff3f5c474a240353993056428fb851e&variables=%7B%22shortcode%22%3A%22${codePage}%22%2C%22include_reel%22%3Atrue%2C%22include_logged_out%22%3Afalse%7D`);
    ...
    async getContentDataById(idContent){
        try {
            const result = await this.request.get(`${this.baseUrl}graphql/query/?query_hash=15bf78a4ad24e33cbd838fdb31353ac1&variables=%7B%22id%22%3A%22${idContent}%22%2C%22first%22%3A12%7D`);
            const jsonResult = await result.json();
    ...
    async getVideoByShortCode(codePage){
        try {
            ...
            const getVideo = await this.request.get(videoUrl.node.video_url);
